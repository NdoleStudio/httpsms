...
func (repository *gormDiscordRepository) DeleteAllForUser(ctx context.Context, userID entities.UserID) error {
	...
	if err := repository.db.WithContext(ctx).Where("user_id = ?", userID).Delete(&entities.Discord{}).Error; err != nil {
		return repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot delete all [%T] for user with ID [%s]", &entities.Discord{}, userID))
	}
...
func (repository *gormDiscordRepository) Save(ctx context.Context, Discord *entities.Discord) error {
	...
	if err := repository.db.WithContext(ctx).Save(Discord).Error; err != nil {
		return repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot update discord integration with ID [%s]", Discord.ID))
	}
...
func (repository *gormDiscordRepository) Index(ctx context.Context, userID entities.UserID, params IndexParams) ([]*entities.Discord, error) {
	...
	if err := query.Order("created_at DESC").Limit(params.Limit).Offset(params.Skip).Find(&discords).Error; err != nil {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot fetch discord integrations for user [%s] and params [%+#v]", userID, params))
	}
...
func (repository *gormDiscordRepository) FetchHavingIncomingChannel(ctx context.Context, userID entities.UserID) ([]*entities.Discord, error) {
	...
	err := repository.db.
		...
		Find(&discords).Error
	if err != nil {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot load discord integrations for user with ID [%s] having a valid [incoming_channel_id]", userID))
	}
...
func (repository *gormDiscordRepository) Load(ctx context.Context, userID entities.UserID, discordID uuid.UUID) (*entities.Discord, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.PropagateWithCodef(err, ErrCodeNotFound, "discord integration with ID [%s] for user [%s] does not exist", discordID, userID))
	}

	if err != nil {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot load discord integration with ID [%s] for user [%s]", discordID, userID))
	}
...
func (repository *gormDiscordRepository) FindByServerID(ctx context.Context, serverID string) (*entities.Discord, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.PropagateWithCodef(err, ErrCodeNotFound, "discord integration with server ID [%s] does not exist", serverID))
	}

	if err != nil {
		return nil, repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot load discord integration with serverID [%s]", serverID))
	}
...
func (repository *gormDiscordRepository) Delete(ctx context.Context, userID entities.UserID, discordID uuid.UUID) error {
	...
	if err != nil {
		return repository.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot delete discord integration with ID [%s] and userID [%s]", discordID, userID))
	}
