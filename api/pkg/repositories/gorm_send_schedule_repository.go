...
func (r *gormSendScheduleRepository) Update(ctx context.Context, schedule *entities.SendSchedule) error {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("schedule_id = ?", schedule.ID).Delete(&entities.SendScheduleWindow{}).Error; err != nil {
			return err
		}
...
func (r *gormSendScheduleRepository) Load(ctx context.Context, userID entities.UserID, scheduleID uuid.UUID) (*entities.SendSchedule, error) {
	...
	err := r.db.WithContext(ctx).Preload("Windows").Where("user_id = ?", userID).Where("id = ?", scheduleID).First(item).Error
...
func (r *gormSendScheduleRepository) Index(ctx context.Context, userID entities.UserID) ([]*entities.SendSchedule, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Windows").Where("user_id = ?", userID).Order("is_default DESC, created_at ASC").Find(&items).Error; err != nil {
...
func (r *gormSendScheduleRepository) Default(ctx context.Context, userID entities.UserID) (*entities.SendSchedule, error) {
	ctx, span := r.tracer.Start(ctx)
	defer span.End()
	item := new(entities.SendSchedule)
	err := r.db.WithContext(ctx).Preload("Windows").Where("user_id = ?", userID).Where("is_default = ?", true).Where("is_active = ?", true).First(item).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, r.tracer.WrapErrorSpan(span, stacktrace.PropagateWithCode(err, ErrCodeNotFound, fmt.Sprintf("default send schedule not found for user [%s]", userID)))
	}
	if err != nil {
		return nil, r.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, fmt.Sprintf("cannot load default send schedule for user [%s]", userID)))
	}
	return item, nil
}

func (r *gormSendScheduleRepository) ClearDefault(ctx context.Context, userID entities.UserID) error {
	ctx, span := r.tracer.Start(ctx)
	defer span.End()
	if err := r.db.WithContext(ctx).Model(&entities.SendSchedule{}).Where("user_id = ?", userID).Update("is_default", false).Error; err != nil {
		return r.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, fmt.Sprintf("cannot clear default schedule for user [%s]", userID)))
	}
	return nil
}
