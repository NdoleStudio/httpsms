...
func (repository *gormUserRepository) RotateAPIKey(ctx context.Context, userID entities.UserID) (*entities.User, error) {
	...
	if err != nil {
		return nil, stacktrace.Propagate(err, "%s", fmt.Sprintf("cannot generate apiKey for user [%s]", userID))
	}
...
func (repository *gormUserRepository) LoadAuthContext(ctx context.Context, apiKey string) (entities.AuthContext, error) {
	...
	if authUser, found := repository.cache.Get(apiKey); found {
		if authUser.IsNoop() {
			return authUser, repository.tracer.WrapErrorSpan(span, stacktrace.NewError("%s", fmt.Sprintf("user with api key [%s] does not exist", apiKey)))
		}
		return authUser, nil
	}
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		return entities.AuthContext{}, repository.tracer.WrapErrorSpan(span, stacktrace.PropagateWithCode(err, ErrCodeNotFound, "%s", msg))
	}

	if err != nil {
		msg := fmt.Sprintf("cannot load user with api key [%s]", apiKey)
		return entities.AuthContext{}, repository.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, "%s", msg))
	}
...
func (repository *gormUserRepository) LoadOrStore(ctx context.Context, authUser entities.AuthContext) (*entities.User, bool, error) {
	...
	apiKey, err := repository.generateAPIKey(64)
	if err != nil {
		return nil, false, stacktrace.Propagate(err, "%s", fmt.Sprintf("cannot generate apiKey for user [%s]", authUser.ID))
	}
	...
	if err != nil {
		...
		return user, isNew, repository.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, "%s", msg))
	}
...
func (repository *gormUserRepository) generateRandomBytes(n int) ([]byte, error) {
	...
	if _, err := rand.Read(b); err != nil {
		return nil, stacktrace.Propagate(err, "%s", fmt.Sprintf("cannot generate [%d] random bytes", n))
	}
