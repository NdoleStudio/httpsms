...
func (listener *MessageThreadListener) OnMessageAPISent(ctx context.Context, event cloudevents.Event) error {
	...
	if err := listener.service.UpdateThread(ctx, updateParams); err != nil {
		return listener.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot update thread for message with ID [%s] for event with ID [%s]", updateParams.MessageID, event.ID()))
	}
...
func (listener *MessageThreadListener) onMessageDeleted(ctx context.Context, event cloudevents.Event) error {
	...
	if err := listener.service.UpdateAfterDeletedMessage(ctx, payload); err != nil {
		return listener.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot update thread for message with ID [%s] for event with ID [%s]", payload.MessageID, event.ID()))
	}
...
func (listener *MessageThreadListener) OnMessagePhoneFailed(ctx context.Context, event cloudevents.Event) error {
	...
	if err := event.DataAs(&payload); err != nil {
		return listener.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot decode [%s] into [%T] for event [%s]", event.Data(), payload, event.ID()))
	}
...
func (listener *MessageThreadListener) OnMessageCallMissed(ctx context.Context, event cloudevents.Event) error {
	...
	if err := listener.service.UpdateThread(ctx, params); err != nil {
		return listener.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot update thread for missed call [%s] on event [%s]", payload.MessageID, event.ID()))
	}
	return nil
}
...
func (listener *MessageThreadListener) onUserAccountDeleted(ctx context.Context, event cloudevents.Event) error {
	...
	if err := listener.service.DeleteAllForUser(ctx, payload.UserID); err != nil {
		return listener.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot delete [entities.MessageThread] for user [%s] on [%s] event with ID [%s]", payload.UserID, event.Type(), event.ID()))
	}
