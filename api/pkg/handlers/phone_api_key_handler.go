...
func (h *PhoneAPIKeyHandler) index(c fiber.Ctx) error {
	...
	if errors := h.validator.ValidateIndex(ctx, request.Sanitize()); len(errors) != 0 {
		ctxLogger.Warn(stacktrace.NewErrorf("validation errors [%s], while fetching phone API keys [%+#v]", spew.Sdump(errors), request))
	...
	if err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot index phone API keys with params [%+#v]", request))
...
func (h *PhoneAPIKeyHandler) store(c fiber.Ctx) error {
	...
	if err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot check entitlement for phone API keys for user [%s]", userID))
		return h.responseInternalServerError(c)
	}
	if !result.Allowed {
...
func (h *PhoneAPIKeyHandler) delete(c fiber.Ctx) error {
	...
	if errors := h.validator.ValidateUUID(phoneAPIKeyID, "phoneAPIKeyID"); len(errors) != 0 {
		ctxLogger.Warn(stacktrace.NewErrorf("validation errors [%s], while deleting a phone API key with ID [%s]", spew.Sdump(errors), phoneAPIKeyID))
	...
	if err != nil {
		ctxLogger.Error(h.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot delete phone API key with ID [%s] for user with ID [%s]", phoneAPIKeyID, h.userIDFomContext(c))))
		return h.responseInternalServerError(c)
	}
...
func (h *PhoneAPIKeyHandler) deletePhone(c fiber.Ctx) error {
	...
	if err != nil {
		ctxLogger.Error(h.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot remove phone with ID [%s] from phone API key with ID [%s] for user with ID [%s]", phoneID, phoneAPIKeyID, h.userIDFomContext(c))))
		return h.responseInternalServerError(c)
	}
