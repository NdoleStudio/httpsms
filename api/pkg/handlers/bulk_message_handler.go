...
func (h *BulkMessageHandler) Index(c fiber.Ctx) error {
	...
	if err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot fetch bulk messages for user [%s]", h.userIDFomContext(c)))
...
func (h *BulkMessageHandler) Store(c fiber.Ctx) error {
	...
	if len(validationErrors) != 0 {
		ctxLogger.Warn(stacktrace.NewErrorf("validation errors [%s], while sending bulk sms from CSV file [%s] for [%s]", spew.Sdump(validationErrors), file.Filename, h.userIDFomContext(c)))
	...
	if msg := h.billingService.IsEntitledWithCount(ctx, h.userIDFomContext(c), uint(len(messages))); msg != nil {
		ctxLogger.Warn(stacktrace.NewErrorf("user with ID [%s] is not entitled to send [%d] messages", h.userIDFomContext(c), len(messages)))
	...
	for _, message := range messages {
		...
		go func(message *requests.BulkMessage, index int) {
			...
			if err != nil {
				...
				ctxLogger.Error(stacktrace.Propagatef(err, "cannot send message with payload [%s] at index [%d]", spew.Sdump(message), index))
			}
			wg.Done()
