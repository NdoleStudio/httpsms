...
func (v *BulkMessageHandlerValidator) ValidateStore(ctx context.Context, userID entities.UserID, header *multipart.FileHeader) ([]*requests.BulkMessage, *time.Location, url.Values) {
	...
	if err != nil {
		...
		ctxLogger.Error(v.tracer.WrapErrorSpan(span, stacktrace.Propagatef(err, "cannot load user [%s]", userID)))
...
func (v *BulkMessageHandlerValidator) parseFile(ctxLogger telemetry.Logger, user *entities.User, header *multipart.FileHeader) ([]*requests.BulkMessage, url.Values) {
	...
	ctxLogger.Error(stacktrace.NewErrorf("cannot parse file [%s] for user [%s] with content type [%s]", header.Filename, user.ID, header.Header.Get("Content-Type")))
...
func (v *BulkMessageHandlerValidator) parseXlsx(ctxLogger telemetry.Logger, user *entities.User, header *multipart.FileHeader) ([]*requests.BulkMessage, url.Values) {
	...
	if err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot generate excel file from [%s] for user [%s]", header.Filename, user.ID))
	...
	if err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot get rows from excel file [%s] for user [%s]", header.Filename, user.ID))
		result.Add("document", fmt.Sprintf("Cannot parse the uploaded excel file with name [%s].", header.Filename))
		return nil, result
	}
...
func (v *BulkMessageHandlerValidator) parseBytes(ctxLogger telemetry.Logger, userID entities.UserID, header *multipart.FileHeader) ([]byte, url.Values) {
	...
	defer func() {
		if e := file.Close(); e != nil {
			ctxLogger.Error(stacktrace.Propagatef(e, "cannot close file [%s] for user [%s]", header.Filename, userID))
		}
	}()

	b := new(bytes.Buffer)
	if _, err = io.Copy(b, file); err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot copy file [%s] to buffer for user [%s]", header.Filename, userID))
...
func (v *BulkMessageHandlerValidator) parseCSV(ctxLogger telemetry.Logger, user *entities.User, header *multipart.FileHeader) ([]*requests.BulkMessage, url.Values) {
	...
	if err := csvutil.Unmarshal(content, &messages); err != nil {
		ctxLogger.Error(stacktrace.Propagatef(err, "cannot unmarshall contents [%s] into type [%T] for file [%s] and user [%s]", content, messages, header.Filename, user.ID))
