...
func (service *MarketingService) DeleteContact(ctx context.Context, email string) error {
	...
	if err != nil {
		return service.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, "%s", fmt.Sprintf("cannot search for contact with email [%s]", email)))
	}
	...
	if _, err = service.plunkClient.Contacts.Delete(ctx, contact.ID); err != nil {
		return service.tracer.WrapErrorSpan(span, stacktrace.Propagate(err, "%s", fmt.Sprintf("cannot delete user with ID [%s] from contacts", contact.Data[string(semconv.EnduserIDKey)])))
	}
